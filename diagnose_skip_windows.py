"""
Check if liquidity ever passes threshold OUTSIDE the skip windows on 2025-04-16.
"""
...
# Track refill opportunities OUTSIDE skip windows
...
# Show periods when both sides pass liquidity check IN TRADABLE WINDOWS
print("\nTIMES WHEN CAN QUOTE (OUTSIDE SKIP WINDOWS):")
print("-"*100)
passing_tradable = [r for r in refill_checks_tradable if r['can_quote']]
if passing_tradable:
    print(f"Found {len(passing_tradable)} tradable opportunities:")
    for r in passing_tradable:
        print(f"{r['time']} | Event #{r['event_num']:>5} | Bid: ${r['bid_liq']:>10,.0f} ✓ | Ask: ${r['ask_liq']:>10,.0f} ✓")
else:
    print("❌ NO TRADABLE OPPORTUNITIES - Liquidity never passes threshold outside skip windows!")

print("\n" + "="*100)
print("\nTIMES WHEN CAN QUOTE (BUT IN SKIP WINDOWS):")
print("-"*100)
passing_skip = [r for r in refill_checks_skip if r['can_quote']]
if passing_skip:
    print(f"Found {len(passing_skip)} opportunities BLOCKED by skip windows:")
    for r in passing_skip:
        print(f"{r['time']} | Event #{r['event_num']:>5} | Bid: ${r['bid_liq']:>10,.0f} ✓ | Ask: ${r['ask_liq']:>10,.0f} ✓ | Window: {r['window_type']}")
else:
    print("None")
...
print(f"  Can quote: {len(passing_tradable)}")
print(f"  Cannot quote: {len(refill_checks_tradable) - len(passing_tradable)}")
print(f"\nSkip window refill checks: {len(refill_checks_skip)}")
print(f"  Can quote (but blocked): {len(passing_skip)}")
print(f"  Cannot quote: {len(refill_checks_skip) - len(passing_skip)}")
print("="*100)

if len(passing_tradable) == 0 and len(passing_skip) > 0:
    print("\n⚠️  DIAGNOSIS: Liquidity only passes threshold during SKIP WINDOWS!")
    print(f"   On 2025-04-16, ADNOCGAS only has sufficient liquidity during {passing_skip[0]['window_type']} window")
    print("   which is excluded from trading by the strategy configuration.")
elif len(passing_tradable) == 0 and len(passing_skip) == 0:
    print("\n⚠️  DIAGNOSIS: Liquidity NEVER passes threshold throughout the day!")
    print(f"   The 13,000 AED threshold is too high for this day's orderbook depth.")
