"""Parameter sweep script for v2_price_follow_qty_cooldown strategy.

This script runs the v2 strategy with different refill_interval_sec
values and compares the performance to find the optimal parameter.

Usage:
    python scripts/sweep_v2_refill_intervals.py
    python scripts/sweep_v2_refill_intervals.py --max-sheets 5  # Quick test
"""
import argparse
import json
import os
...
def run_with_interval(interval_sec: int, base_config: dict, data_path: str, 
    ...
    """
    print(f"\n{'='*80}")
    print(f"Testing V2 Refill Interval: {interval_sec} seconds ({interval_sec/60:.1f} minutes)")
    print(f"{'='*80}")
...
def plot_comparison(comparison_df: pd.DataFrame, output_path: str):
    ...
    fig.suptitle('V2 Strategy: Refill Interval Parameter Sweep', fontsize=16, fontweight='bold')
...
def main():
    parser = argparse.ArgumentParser(
        description='Parameter sweep for v2_price_follow_qty_cooldown strategy'
    )
    parser.add_argument(
        '--config',
        type=str,
        default='configs/mm_config.json',
        help='Path to base config file (default: configs/mm_config.json)'
    )
    parser.add_argument(
        '--data',
        type=str,
        default='data/MarketDepth_Sampled.xlsx',
        help='Path to data file (default: data/MarketDepth_Sampled.xlsx)'
    )
    parser.add_argument(
        '--intervals',
        type=int,
    ...
    parser.add_argument(
        '--output-dir',
        type=str,
        default='output/v2_parameter_sweep',
        help='Output directory (default: output/v2_parameter_sweep)'
    )
    
    args = parser.parse_args()
    
    print(f"\n{'='*80}")
    print("V2 STRATEGY: REFILL INTERVAL PARAMETER SWEEP")
    print(f"{'='*80}")
    print(f"Strategy: v2_price_follow_qty_cooldown")
    ...
    # Save comparison CSV
    comparison_path = os.path.join(args.output_dir, 'v2_interval_comparison.csv')
    comparison_df.to_csv(comparison_path, index=False)
    print(f"\n{'='*80}")
    print("V2 COMPARISON SUMMARY")
    print(f"{'='*80}")
    ...
    plot_path = os.path.join(args.output_dir, 'v2_interval_comparison.png')
